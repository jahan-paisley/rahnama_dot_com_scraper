...
class PlainScraperForRental

  def initialize
    @results = {}
    @server = "http://www.rahnama.com/cat/index/id/34851/%D8%A7%D8%AC%D8%A7%D8%B1%D9%87-%D8%A7%D9%85%D9%84%D8%A7%D9%83-%D9%85%D8%B3%D9%83%D9%88%D9%86%D9%8A"
  end

  def start
    # doc = Nokogiri::HTML(open(@server))
    # href = doc.css("a[href*='فروش-املاك-مسكوني']").map { |e| e['href'] }.first
    home_doc= Nokogiri::HTML(open(@server))
    links= IO.readlines('config/links_for_rental.txt').map(&:chomp)
    links.each do |link|
